...
public:
    ...
    GameplayInputSample()  {}
    GameplayInputSample(float player_x, float player_y, float player_w, float player_h, float move_speed, float axis_x, float axis_y, std::string player_color, std::string status_text) : player_x(player_x), player_y(player_y), player_w(player_w), player_h(player_h), move_speed(move_speed), axis_x(axis_x), axis_y(axis_y), player_color(player_color), status_text(status_text) {}

    void init() {
        println("GameplayInputSample initialized");
    }
    ...
    void draw(zenith::Canvas& canvas) {
        ...
        canvas.clear("black");
        canvas.drawRect(player_x, player_y, player_w, player_h, player_color);
        canvas.drawText("Gameplay Input Sample", 2.0, 1.0, "yellow");
...
int main() {
    ...
    GameplayInputSample game = GameplayInputSample();
