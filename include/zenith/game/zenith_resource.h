...
private:
    ...
    std::unordered_map<int, AssetHandle> handlesById;
    ...
    AssetHandle& ensureHandle(
        ...
        it->second.path = path;
        it->second.type = type;
        handlesById[it->second.id] = it->second;
        return it->second;
    }
    ...
    const AssetHandle* findHandleById(int id, AssetType expectedType) const {
        if (id <= 0) {
            return nullptr;
        }
        ...
        if (expectedType != AssetType::Unknown && it->second.type != expectedType) {
            return nullptr;
        }
        return &it->second;
    }

    std::string resolveHandlePath(const AssetHandle& handle, AssetType expectedType) const {
        if (const AssetHandle* resolved = findHandleById(handle.id, expectedType)) {
            return resolved->path;
        }
        if (handle.path.empty()) {
            return "";
        }
        return normalizeAssetPath(handle.path);
    }
...
public:
    ...
    std::shared_ptr<TextureAsset> loadTexture(const AssetHandle& handle) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Texture);
        return resolvedPath.empty() ? nullptr : loadTexture(resolvedPath);
    }
    ...
    std::shared_ptr<AudioAsset> loadAudio(const AssetHandle& handle, bool isSpatial = false) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Audio);
        return resolvedPath.empty() ? nullptr : loadAudio(resolvedPath, isSpatial);
    }
    ...
    std::shared_ptr<MeshAsset> loadMesh(const AssetHandle& handle) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Mesh);
        return resolvedPath.empty() ? nullptr : loadMesh(resolvedPath);
    }
    ...
    std::shared_ptr<ShaderAsset> loadShader(const AssetHandle& handle) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Shader);
        return resolvedPath.empty() ? nullptr : loadShader(resolvedPath);
    }
    ...
    std::shared_ptr<MaterialAsset> loadMaterial(const AssetHandle& handle, const std::string& shaderPath = "") {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Material);
        return resolvedPath.empty() ? nullptr : loadMaterial(resolvedPath, shaderPath);
    }
    ...
    AssetHandle loadTextureHandle(const AssetHandle& handle) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Texture);
        return resolvedPath.empty() ? AssetHandle{} : loadTextureHandle(resolvedPath);
    }
    ...
    AssetHandle loadAudioHandle(const AssetHandle& handle, bool isSpatial = false) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Audio);
        return resolvedPath.empty() ? AssetHandle{} : loadAudioHandle(resolvedPath, isSpatial);
    }
    ...
    AssetHandle loadMeshHandle(const AssetHandle& handle) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Mesh);
        return resolvedPath.empty() ? AssetHandle{} : loadMeshHandle(resolvedPath);
    }
    ...
    AssetHandle loadShaderHandle(const AssetHandle& handle) {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Shader);
        return resolvedPath.empty() ? AssetHandle{} : loadShaderHandle(resolvedPath);
    }
    ...
    AssetHandle loadMaterialHandle(const AssetHandle& handle, const std::string& shaderPath = "") {
        const std::string resolvedPath = resolveHandlePath(handle, AssetType::Material);
        return resolvedPath.empty() ? AssetHandle{} : loadMaterialHandle(resolvedPath, shaderPath);
    }
