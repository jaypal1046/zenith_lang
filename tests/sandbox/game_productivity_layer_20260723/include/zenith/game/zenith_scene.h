...
public:
    ...
    bool loadSceneStream(const std::string& streamName, const std::string& instanceName = "") {
        ...
        instance.entities = collectNewEntities(before);
    ...
    bool unloadSceneStream(const std::string& instanceName) {
        ...
        std::vector<EntityId> entities = it->second.entities;
    ...
    Sprite2DView sprite2D(EntityId entity) {
        ...
        view.texture = FieldRef<TextureHandleView>(
            [this, entity]() {
                if (const SpriteRenderer2D* sprite = world.getSpriteRenderer2D(entity)) {
                    return makeTextureHandleView(sprite->texture);
                }
                return TextureHandleView{};
            },
            [this, entity](const TextureHandleView& value) {
                if (SpriteRenderer2D* sprite = ensureSprite2DViewComponent(entity)) {
                    sprite->texture = value.path.empty()
                        ? nullptr
                        : resource::ResourceManager::getInstance().loadTexture(value.path);
                }
            });
    ...
    AudioSource2DView audioSource2D(EntityId entity) {
        ...
        view.clip = FieldRef<AudioHandleView>(
            [this, entity]() {
                if (const AudioSource2DComponent* audio = world.getAudioSource2D(entity)) {
                    return makeAudioHandleView(audio->clip);
                }
                return AudioHandleView{};
            },
            [this, entity](const AudioHandleView& value) {
                if (AudioSource2DComponent* audio = ensureAudioSource2DViewComponent(entity)) {
                    audio->clip = value.path.empty()
                        ? nullptr
                        : resource::ResourceManager::getInstance().loadAudio(value.path, false);
                }
            });
    ...
    TextureHandleView loadTexture(std::string texturePath) {
        return makeTextureHandleView(resource::ResourceManager::getInstance().loadTexture(texturePath));
    }

    AudioHandleView loadAudio(std::string clipPath, bool spatial) {
        return makeAudioHandleView(resource::ResourceManager::getInstance().loadAudio(clipPath, spatial));
    }

    MeshHandleView loadMesh(std::string meshPath) {
        return makeMeshHandleView(resource::ResourceManager::getInstance().loadMesh(meshPath));
    }

    ShaderHandleView loadShader(std::string shaderPath) {
        return makeShaderHandleView(resource::ResourceManager::getInstance().loadShader(shaderPath));
    }

    MaterialHandleView loadMaterial(std::string materialPath, std::string shaderPath) {
        return makeMaterialHandleView(resource::ResourceManager::getInstance().loadMaterial(materialPath, shaderPath));
    }
    ...
    EntityId spawnTexturedSpriteHandle(std::string name, TextureHandleView texture, float x, float y, float w, float h, std::string color) {
        return spawnTexturedSprite(name, texture.path, x, y, w, h, color);
    }
    ...
    EntityId spawnCharacter2DHandle(std::string name, TextureHandleView texture, float x, float y, float w, float h, std::string color) {
        return spawnCharacter2D(name, texture.path, x, y, w, h, color);
    }
    ...
    EntityId spawnAudioSource2DHandle(std::string name, AudioHandleView clip, float x, float y, bool playOnAwake) {
        return spawnAudioSource2D(name, clip.path, x, y, playOnAwake);
    }
    ...
    EntityId spawnMeshHandle(std::string name, MeshHandleView mesh, ShaderHandleView shader, float x, float y, float z) {
        return spawnMesh(name, mesh.path, shader.path, x, y, z);
    }
    ...
    EntityId spawnCharacter3DHandle(std::string name, MeshHandleView mesh, ShaderHandleView shader, MaterialHandleView material, float x, float y, float z) {
        return spawnCharacter3D(name, mesh.path, shader.path, material.path, x, y, z);
    }
    ...
    MaterialHandleView meshMaterialHandle(EntityId entity) {
        if (const MeshRenderer3D* mesh = world.getMeshRenderer3D(entity)) {
            return makeMaterialHandleView(mesh->material);
        }
    ...
    Mesh3DView mesh3D(EntityId entity) {
        ...
        view.mesh = FieldRef<MeshHandleView>(
            [this, entity]() {
                if (const MeshRenderer3D* mesh = world.getMeshRenderer3D(entity)) {
                    return makeMeshHandleView(mesh->mesh);
                }
                return MeshHandleView{};
            },
            [this, entity](const MeshHandleView& value) {
                if (MeshRenderer3D* mesh = ensureMesh3DViewComponent(entity)) {
                    mesh->mesh = value.path.empty()
                        ? nullptr
                        : resource::ResourceManager::getInstance().loadMesh(value.path);
                }
            });
        ...
        view.shader = FieldRef<ShaderHandleView>(
            [this, entity]() {
                if (const MeshRenderer3D* mesh = world.getMeshRenderer3D(entity)) {
                    return makeShaderHandleView(mesh->shader);
                }
                return ShaderHandleView{};
            },
            [this, entity](const ShaderHandleView& value) {
                if (MeshRenderer3D* mesh = ensureMesh3DViewComponent(entity)) {
                    mesh->shader = value.path.empty()
                        ? nullptr
                        : resource::ResourceManager::getInstance().loadShader(value.path);
                }
            });
        ...
        view.material = FieldRef<MaterialHandleView>(
            ...
            [this, entity](const MaterialHandleView& value) {
                if (MeshRenderer3D* mesh = ensureMesh3DViewComponent(entity)) {
                    mesh->material = value.path.empty()
                        ? nullptr
                        : resource::ResourceManager::getInstance().loadMaterial(value.path, value.shaderPath);
                }
            });
        ...
        view.clip = FieldRef<AudioHandleView>(
            ...
            [this, entity](const AudioHandleView& value) {
                if (AudioSource3DComponent* audio = ensureAudioSource3DViewComponent(entity)) {
                    audio->clip = value.path.empty()
                        ? nullptr
                        : resource::ResourceManager::getInstance().loadAudio(value.path, true);
                }
            });
    ...
    EntityId spawnAudioSource3DHandle(std::string name, AudioHandleView clip, float x, float y, float z, bool playOnAwake) {
        return spawnAudioSource3D(name, clip.path, x, y, z, playOnAwake);
    }
...
private:
    TextureHandleView makeTextureHandleView(const std::shared_ptr<resource::TextureAsset>& texture) const {
        if (!texture) {
            return TextureHandleView{};
        }
        resource::AssetHandle handle = resource::ResourceManager::getInstance().loadTextureHandle(texture->path);
        TextureHandleView view;
        view.path = handle.path;
        view.id = handle.id;
        view.refCount = handle.refCount;
        view.loaded = handle.isLoaded;
    ...
    AudioHandleView makeAudioHandleView(const std::shared_ptr<resource::AudioAsset>& audio) const {
        if (!audio) {
            return AudioHandleView{};
        }
        resource::AssetHandle handle = resource::ResourceManager::getInstance().loadAudioHandle(audio->path, audio->isSpatial);
    ...
    MeshHandleView makeMeshHandleView(const std::shared_ptr<resource::MeshAsset>& mesh) const {
        if (!mesh) {
            return MeshHandleView{};
        }
        resource::AssetHandle handle = resource::ResourceManager::getInstance().loadMeshHandle(mesh->path);
        MeshHandleView view;
        view.path = handle.path;
        view.id = handle.id;
        view.refCount = handle.refCount;
    ...
    ShaderHandleView makeShaderHandleView(const std::shared_ptr<resource::ShaderAsset>& shader) const {
        if (!shader) {
            return ShaderHandleView{};
        }
        resource::AssetHandle handle = resource::ResourceManager::getInstance().loadShaderHandle(shader->path);
    ...
    MaterialHandleView makeMaterialHandleView(const std::shared_ptr<resource::MaterialAsset>& material) const {
        if (!material) {
            return MaterialHandleView{};
        }
        resource::AssetHandle handle = resource::ResourceManager::getInstance().loadMaterialHandle(material->path, material->shaderPath);
