...
class ControllerScene : public zenith::game::Scene {
private:
public:
    zenith::game::EntityId floor;
    zenith::game::EntityId wall;
    zenith::game::EntityId player;

    ControllerScene()  {}
    ControllerScene(zenith::game::EntityId floor, zenith::game::EntityId wall, zenith::game::EntityId player) : floor(floor), wall(wall), player(player) {}
    ...
    void onLoad() {
        ...
        zenith::game::Character3DView controller = character3D(player);
...
int main() {
    ...
    zenith::game::Character3DView controller = scene.character3D(scene.player);
    zenith::game::Body3DView body = scene.body3D(scene.player);
    std::string landed = "landed=";
    landed = zenith::concat(landed, controller.isGrounded);
    println(landed);
    std::string landed_y = "landed_y_ok=";
    landed_y = zenith::concat(landed_y, controller.y >= 0.35);
    println(landed_y);
    ...
    std::string wall_blocked = "wall_blocked=";
    wall_blocked = zenith::concat(wall_blocked, controller.x <= 0.6);
    println(wall_blocked);
    std::string slide_forward = "slide_forward=";
    slide_forward = zenith::concat(slide_forward, controller.z > 1.5);
    ...
    scene.simulateFixedStep(scene.fixedDeltaTime);
    std::string jump_air = "jump_air=";
    jump_air = zenith::concat(jump_air, !controller.isGrounded && body.vy > 0.0);
    println(jump_air);
    i = 0;
    while (i < 240) {
        scene.simulateFixedStep(scene.fixedDeltaTime);
    ...
    std::string relanded = "relanded=";
    relanded = zenith::concat(relanded, controller.isGrounded && body.isGrounded);
    println(relanded);
