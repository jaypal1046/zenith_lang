...
public:
    ...
    RaycastHit2DResult raycast2D(
        ...
        for (size_t i = 0; i < boxColliders2D.size(); ++i) {
            EntityId entity = boxColliders2D.entityAt(i);
            if (!layerMaskMatches(layerMask, entityLayerBits(entity))) {
                continue;
            }
            const physics::BoxCollider2D& collider = boxColliders2D.componentAt(i);
            float distance = 0.0f;
            physics::Vec2 point;
            physics::Vec2 normal;
        ...
        for (size_t i = 0; i < circleColliders2D.size(); ++i) {
            EntityId entity = circleColliders2D.entityAt(i);
            if (!layerMaskMatches(layerMask, entityLayerBits(entity))) {
                continue;
            }
            const physics::CircleCollider2D& collider = circleColliders2D.componentAt(i);
            float distance = 0.0f;
            physics::Vec2 point;
            physics::Vec2 normal;
            if (physics::raycast(ray, collider, entityPosition2D(entity), bestDistance, distance, point, normal)) {
        ...
        for (size_t i = 0; i < capsuleColliders2D.size(); ++i) {
            EntityId entity = capsuleColliders2D.entityAt(i);
            if (!layerMaskMatches(layerMask, entityLayerBits(entity))) {
                continue;
            }
            const physics::CapsuleCollider2D& collider = capsuleColliders2D.componentAt(i);
            float distance = 0.0f;
            physics::Vec2 point;
            physics::Vec2 normal;
            if (physics::raycast(ray, collider, entityPosition2D(entity), bestDistance, distance, point, normal)) {
    ...
    RaycastHit3DResult raycast3D(
        ...
        for (size_t i = 0; i < boxColliders3D.size(); ++i) {
            EntityId entity = boxColliders3D.entityAt(i);
            if (!layerMaskMatches(layerMask, entityLayerBits(entity))) {
                continue;
            }
            const physics::BoxCollider3D& collider = boxColliders3D.componentAt(i);
            float distance = 0.0f;
            physics::Vec3 point;
            physics::Vec3 normal;
        ...
        for (size_t i = 0; i < sphereColliders3D.size(); ++i) {
            EntityId entity = sphereColliders3D.entityAt(i);
            if (!layerMaskMatches(layerMask, entityLayerBits(entity))) {
                continue;
            }
            const physics::SphereCollider3D& collider = sphereColliders3D.componentAt(i);
            float distance = 0.0f;
            physics::Vec3 point;
            physics::Vec3 normal;
            if (physics::raycast(ray, collider, entityPosition3D(entity), bestDistance, distance, point, normal)) {
    ...
    void syncPhysicsFromTransforms2D() {
        for (size_t i = 0; i < rigidBodies2D.size(); ++i) {
            EntityId entity = rigidBodies2D.entityAt(i);
            Transform2D* transform = transforms2D.get(entity);
            physics::RigidBody2D& body = rigidBodies2D.componentAt(i);
    ...
    void syncTransformsFromPhysics2D() {
        for (size_t i = 0; i < rigidBodies2D.size(); ++i) {
            EntityId entity = rigidBodies2D.entityAt(i);
            Transform2D* transform = transforms2D.get(entity);
            const physics::RigidBody2D& body = rigidBodies2D.componentAt(i);
            if (transform == nullptr) {
                continue;
            }
    ...
    void syncPhysicsFromTransforms3D() {
        for (size_t i = 0; i < rigidBodies3D.size(); ++i) {
            EntityId entity = rigidBodies3D.entityAt(i);
            Transform3D* transform = transforms3D.get(entity);
            physics::RigidBody3D& body = rigidBodies3D.componentAt(i);
            if (transform == nullptr) {
                continue;
            }
            body.position = transform->position;
    ...
    void syncTransformsFromPhysics3D() {
        for (size_t i = 0; i < rigidBodies3D.size(); ++i) {
            EntityId entity = rigidBodies3D.entityAt(i);
            Transform3D* transform = transforms3D.get(entity);
            const physics::RigidBody3D& body = rigidBodies3D.componentAt(i);
            if (transform == nullptr) {
                continue;
            }
            transform->position = body.position;
...
private:
    ...
    static void forEachComponent(ComponentPool<T>& pool, Fn&& fn) {
        for (size_t i = 0; i < pool.size(); ++i) {
            fn(pool.entityAt(i), pool.componentAt(i));
        }
    }
